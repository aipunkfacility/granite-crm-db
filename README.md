...
**Требования:** Python 3.10+, Node.js 18+ (для Firecrawl CLI)
...
│   │   ├── firecrawl_client.py # Firecrawl search/scrape
...
| **`pipeline_runs`** | ⚠️ **DEPRECATED** — не используется. Чекпоинты работают через подсчёт записей в таблицах. | — |

### Связи

```
raw_companies.merged_from ──→ companies.id       (many-to-one, merged_from = JSON list)
raw_companies.merged_into   ──→ ⚠️ DEPRECATED (не используется)
...
Конвейер запоминает прогресс в БД. При перезапуске — продолжает с прерванного этапа. Логика работает через подсчёт записей в таблицах (не через `pipeline_runs`):
...
### Firecrawl CLI не установлен
При запуске с firecrawl будет предупреждение, но основной функционал (jsprav, скреперы) продолжит работать. Установите:
```bash
npm install -g firecrawl-cli
firecrawl login
```
...
### Структура модулей
- `granite/scrapers/` — парсеры источников (jsprav, dgis, firecrawl...)
...
169 тестов покрывают: дедупликацию, классификатор, обогащение (TG finder, TG trust, tech extractor, messenger scanner), экспорт (CSV, Markdown, пресеты), скреперы, утилиты, миграции БД.
